<template>
	<view>
		...
		次数：{{ num }} 型号：{{ os }} 次数2：{{ runData.steps }}
		<!-- :circles="circles" -->
		<map
			id="myMap"
			v-if="runType == 0"
					...
					<corver-view class="cu-item shadow">
						<corver-view class="flex padding-left padding-right justify-center">
							<corver-view class="text-center">
								...
								<!-- <bing-countup
									class="flex margin-left text-xxl text-bold"
									ref="time"
									:hour="time.hour"
									:minute="time.minute"
									:second="time.second"
...
export default {
	...
	onShow: function() {
		...
		/* console.log("App显示" + this.motion_status + this.hideDateTime);
		if(this.motion_status == 1 && this.hideDateTime != null){
			let timeDiff = difference(this.hideDateTime, new Date());
			console.log(timeDiff);
			this.time = add(this.hideTimeDate, timeDiff);
	...
	onHide: function() {
		...
		/* if(this.motion_status == 1){
			...
			// 记录隐藏App开始时间
			this.hideDateTime = new Date();
			this.hideTimeDate = this.timeData;
		} */
		if (this.motion_status == 1) {
			// 记录隐藏App开始时间
	...
	methods: {
		...
		onChange(e) {
			this.timeData = e;
			console.log(this.timeData.hour.toString() + ":" + this.timeData.minute.toString() + ":" + this.timeData.second.toString());
		},
		/**
		 * 计算加速计数据
		 */
