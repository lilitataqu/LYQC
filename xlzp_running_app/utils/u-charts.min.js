...
function createCurveControlPoints(e, t) {
  ...
  if (1 > t ? (n = e[0].x + (e[1].x - e[0].x) * o, l = e[0].y + (e[1].y - e[0].y) * o) : (n = e[t].x + (e[t + 1].x - e[t - 1].x) * o, l = e[t].y + (e[t + 1].y - e[t - 1].y) * o), t > e.length - 3) {
    var d = e.length - 1;
    r = e[d].x - (e[d].x - e[d - 1].x) * a, s = e[d].y - (e[d].y - e[d - 1].y) * a;
  } else r = e[t + 1].x - (e[t + 2].x - e[t].x) * a, s = e[t + 1].y - (e[t + 2].y - e[t].y) * a;

  return i(e, t + 1) && (s = e[t + 1].y), i(e, t) && (l = e[t].y), {
    ctrA: {
      x: n,
      y: l
    },
...
function avoidCollision(e, t) {
  if (t) for (; util.isCollision(e, t);) 0 < e.start.x ? e.start.y-- : 0 > e.start.x ? e.start.y++ : 0 < e.start.y ? e.start.y++ : e.start.y--;
  return e;
}
...
function measureText(e) {
  ...
  for (let t, o = 0; o < e.length; o++) t = e[o], a += /[a-zA-Z]/.test(t) ? 7 : /[0-9]/.test(t) ? 5.5 : /\./.test(t) ? 2.7 : /-/.test(t) ? 3.25 : /[\u4e00-\u9fa5]/.test(t) ? 10 : /\(|\)/.test(t) ? 3.73 : /\s/.test(t) ? 2.5 : /%/.test(t) ? 8 : 10;
...
function getSeriesDataItem(e, t) {
  ...
  for (let a, o = 0; o < e.length; o++) if (a = e[o], null !== a.data[t] && "undefined" != typeof a.data[t] && a.show) {
    let e = {};
    e.color = a.color, e.type = a.type, e.style = a.style, e.pointShape = a.pointShape, e.disableLegend = a.disableLegend, e.name = a.name, e.show = a.show, e.data = a.format ? a.format(a.data[t]) : a.data[t], i.push(e);
  }
...
function findCurrentIndex(e, t, i, a) {
  var o = 4 < arguments.length && void 0 !== arguments[4] ? arguments[4] : 0,
      n = -1,
      l = 0;
  return ("line" == i.type || "area" == i.type) && "justify" == i.xAxis.boundaryGap && (l = i.chartData.eachSpacing / 2), isInExactChartArea(e, i, a) && t.forEach(function (t, i) {
    e.x + o + l > t && (n = i);
  }), n;
}
...
function findFunnelChartCurrentIndex(e, t) {
  for (var a, o = -1, n = 0, l = t.series.length; n < l; n++) if (a = t.series[n], e.x > a.funnelArea[0] && e.x < a.funnelArea[2] && e.y > a.funnelArea[1] && e.y < a.funnelArea[3]) {
    o = n;
    break;
  }
...
function findWordChartCurrentIndex(e, t) {
  for (var a, o = -1, n = 0, l = t.length; n < l; n++) if (a = t[n], e.x > a.area[0] && e.x < a.area[2] && e.y > a.area[1] && e.y < a.area[3]) {
    o = n;
    break;
  }
...
function findMapChartCurrentIndex(e, t) {
  for (var a, o = -1, n = t.chartData.mapData, l = t.series, r = pointToCoordinate(e.y, e.x, n.bounds, n.scale, n.xoffset, n.yoffset), s = [r.x, r.y], d = 0, h = l.length; d < h; d++) if (a = l[d].geometry.coordinates, isPoiWithinPoly(s, a)) {
    o = d;
    break;
  }
...
function findPieChartCurrentIndex(e, t) {
  ...
  if (isInExactPieChartArea(e, t.center, t.radius)) {
    ...
    for (var n, l = 0, r = t.series.length; l < r; l++) if (n = t.series[l], isInAngleRange(o, n._start_, n._start_ + 2 * n._proportion_ * Math.PI)) {
      a = l;
      break;
    }
  }
...
function calLegendData(e, t, i, a) {
  ...
  if ("top" == t.legend.position || "bottom" == t.legend.position) {
    ...
    if (g.length) {
      ...
      l.area.width = e + 2 * r, l.area.wholeWidth = e + 2 * r, l.area.height = a.length * c + 2 * r, l.area.wholeHeight = a.length * c + 2 * r + 2 * s, l.points = a;
    }
  } else {
    let i = e.length,
...
function getPieDataPoints(e, t) {
  ...
  for (let n, l = 0; l < e.length; l++) n = e[l], n.data = null === n.data ? 0 : n.data, n._proportion_ = 0 === o ? 1 / e.length * a : n.data / o * a, n._radius_ = t;
...
function getRoseDataPoints(e, t, a, o) {
  ...
  for (let r, s = 0; s < e.length; s++) r = e[s], r.data = null === r.data ? 0 : r.data, 0 === l || "area" == t ? (r._proportion_ = r.data / l * n, r._rose_proportion_ = 1 / e.length * n) : (r._proportion_ = r.data / l * n, r._rose_proportion_ = r.data / l * n), r._radius_ = a + (o - a) * ((r.data - d) / (h - d));
...
function getGaugeAxisPoints(e, t, a) {
  ...
  for (let n = 0; n < e.length; n++) e[n].value = null === e[n].value ? 0 : e[n].value, e[n]._startAngle_ = o, e[n]._endAngle_ = (t - a + 1) * e[n].value + t, 2 <= e[n]._endAngle_ && (e[n]._endAngle_ %= 2), o = e[n]._endAngle_;
...
function getGaugeDataPoints(e, t, a) {
  ...
  for (let n, l = 0; l < e.length; l++) {
    if (n = e[l], n.data = null === n.data ? 0 : n.data, "auto" == a.pointer.color) {
      for (let e = 0; e < t.length; e++) if (n.data <= t[e].value) {
        n.color = t[e].color;
        break;
      }
...
function getDataPoints(e, t, i, a, o, n) {
  ...
  return e.forEach(function (e, x) {
    if (null === e) d.push(null);else {
      var c = {};
      c.color = e.color, c.x = a[x], "center" == s && (c.x += l(o / 2));
      var p = e;
      "object" == typeof e && null != e && (p = e.value);
      var g = h * (p - t) / (i - t);
      g *= r, c.y = n.height - l(g) - n.area[2], d.push(c);
    }
  }), d;
}
...
function getYAxisTextList(e, t, a, o) {
  ...
  n = n.filter(function (e) {
    ...
    "object" == typeof e ? e.constructor == Array ? e.map(function (e) {
      d.push(e);
    }) : d.push(e.value) : d.push(e);
  });
  var h = 0,
      x = 0;
  ...
  for (var p = getDataRange(h, x), g = p.minRange, y = p.maxRange, f = [], u = (y - g) / a.yAxisSplit, m = 0; m <= a.yAxisSplit; m++) f.push(g + u * m);
...
function drawPointText(e, t, i, a) {
  ...
  e.forEach(function (e, n) {
    if (null !== e) {
      ...
      var l = o[n];
      "object" == typeof o[n] && null !== o[n] && (l = o[n].value);
...
function drawPieText(e, t, a, o, i, n) {
  ...
  for (let h = 0; h < g.length; h++) {
    let e = g[h],
        ...
        f = i,
        u = measureText(e.text),
        m = f;
    p && util.isSameXCoordinateArea(p.start, {
      x: y
    }) && (0 < y ? m = s(f, p.start.y) : 0 > t ? m = d(f, p.start.y) : 0 < f ? m = d(f, p.start.y) : m = s(f, p.start.y)), 0 > y && (y -= u);
    let S = {
      lineStart: {
        x: o,
        y: n
  ...
  for (let l = 0; l < c.length; l++) {
    ...
    o.setLineWidth(1 * t.pixelRatio), o.setFontSize(a.fontSize), o.beginPath(), o.setStrokeStyle(e.color), o.setFillStyle(e.color), o.moveTo(i.x, i.y);
    let d = 0 > e.start.x ? s.x + e.width : s.x,
        x = 0 > e.start.x ? s.x - 5 : s.x + 5;
    o.quadraticCurveTo(r.x, r.y, d, s.y), o.moveTo(i.x, i.y), o.stroke(), o.closePath(), o.beginPath(), o.moveTo(s.x + e.width, s.y), o.arc(d, s.y, 2, 0, 2 * h), o.closePath(), o.fill(), o.beginPath(), o.setFontSize(e.textSize || a.fontSize), o.setFillStyle(e.textColor || "#666666"), o.fillText(e.text, x, s.y + 3), o.closePath(), o.stroke(), o.closePath();
  }
}
...
function drawMarkLine(e, t, a) {
  ...
  for (let s, d = 0; d < r.length; d++) if (s = assign({}, {
    lineColor: "#DE4A42",
    showLabel: !1,
  ...
  return t._scrollDistance_ && 0 !== t._scrollDistance_ && !0 === t.enableScroll && (a.translate(t._scrollDistance_, 0), h = Math.floor(-t._scrollDistance_ / r) - 2, x = h + t.xAxis.itemCount + 4), t.tooltip && t.tooltip.textList && t.tooltip.textList.length && 1 === o && drawToolTipSplitArea(t.tooltip.offset.x, t, i, a, r), e.forEach(function (n, c) {
    ...
    switch (s.type) {
      case "group":
        ...
        for (let e, o = 0; o < u.length; o++) if (e = u[o], null !== e && o > h && o < x) {
          a.beginPath(), a.setStrokeStyle(e.color || n.color), a.setLineWidth(1), a.setFillStyle(e.color || n.color);
          var S = e.x - e.width / 2,
              T = t.height - e.y - t.area[2];
          a.moveTo(S - 1, e.y), a.lineTo(S + e.width - 2, e.y), a.lineTo(S + e.width - 2, t.height - t.area[2]), a.lineTo(S, t.height - t.area[2]), a.lineTo(S, e.y), a.closePath(), a.stroke(), a.fill();
        }
      ...
      case "meter":
        ...
        if (d.push(u), u = fixColumeMeterData(u, r, e.length, c, i, t, s.meter.border), 0 == c) {
          for (let e, o = 0; o < u.length; o++) if (e = u[o], null !== e && o > h && o < x) {
            ...
            var S = e.x - e.width / 2,
                T = t.height - e.y - t.area[2];
            a.moveTo(S, e.y), a.fillRect(S, e.y, e.width, T), a.closePath(), a.fill(), 0 < s.meter.border && (a.beginPath(), a.setStrokeStyle(n.color), a.setLineWidth(s.meter.border * t.pixelRatio), a.moveTo(S + .5 * s.meter.border, e.y + T), a.lineTo(S + .5 * s.meter.border, e.y + .5 * s.meter.border), a.lineTo(S + e.width - .5 * s.meter.border, e.y + .5 * s.meter.border), a.lineTo(S + e.width - .5 * s.meter.border, e.y + T), a.stroke());
          }
        } else for (let e, o = 0; o < u.length; o++) if (e = u[o], null !== e && o > h && o < x) {
          a.beginPath(), a.setFillStyle(e.color || n.color);
          var S = e.x - e.width / 2,
              T = t.height - e.y - t.area[2];
...
function drawCandleDataPoints(e, t, a, i, o) {
  ...
  return a._scrollDistance_ && 0 !== a._scrollDistance_ && !0 === a.enableScroll && (o.translate(a._scrollDistance_, 0), x = Math.floor(-a._scrollDistance_ / d) - 2, c = x + a.xAxis.itemCount + 4, p = -a._scrollDistance_ - d + a.area[3], g = p + (a.xAxis.itemCount + 4) * d), l.average.show && t.forEach(function (e) {
    ...
    for (let t, a = 0; a < c.length; a++) {
      if (t = c[a], o.beginPath(), o.setStrokeStyle(e.color), o.setLineWidth(1), 1 === t.length) o.moveTo(t[0].x, t[0].y), o.arc(t[0].x, t[0].y, 1, 0, 2 * Math.PI);else {
        o.moveTo(t[0].x, t[0].y);
        let e = 0;

        for (let i, a = 0; a < t.length; a++) if (i = t[a], 0 == e && i.x > p && (o.moveTo(i.x, i.y), e = 1), 0 < a && i.x > p && i.x < g) {
  ...
  return t._scrollDistance_ && 0 !== t._scrollDistance_ && !0 === t.enableScroll && (a.translate(t._scrollDistance_, 0), x = -t._scrollDistance_ - s + t.area[3], c = x + (t.xAxis.itemCount + 4) * s), e.forEach(function (e) {
    ...
    for (let o, l = 0; l < u.length; l++) {
      if (o = u[l], a.beginPath(), a.setStrokeStyle(hexToRgb(e.color, n.opacity)), n.gradient) {
        ...
        i.addColorStop("0", hexToRgb(e.color, n.opacity)), i.addColorStop("1.0", hexToRgb("#FFFFFF", .1)), a.setFillStyle(i);
      ...
      if (a.setLineWidth(n.width * t.pixelRatio), 1 < o.length) {
        ...
        if ("curve" === n.type) {
          for (let e, t = 0; t < o.length; t++) if (e = o[t], 0 == i && e.x > x && (a.moveTo(e.x, e.y), i = 1), 0 < t && e.x > x && e.x < c) {
            let i = createCurveControlPoints(o, t - 1);
            a.bezierCurveTo(i.ctrA.x, i.ctrA.y, i.ctrB.x, i.ctrB.y, e.x, e.y);
          }
        } else for (let e, t = 0; t < o.length; t++) e = o[t], 0 == i && e.x > x && (a.moveTo(e.x, e.y), i = 1), 0 < t && e.x > x && e.x < c && a.lineTo(e.x, e.y);

        a.lineTo(t.x, d), a.lineTo(e.x, d), a.lineTo(e.x, e.y);
      } else {
        let e = o[0];
        a.moveTo(e.x - s / 2, e.y), a.lineTo(e.x + s / 2, e.y), a.lineTo(e.x + s / 2, d), a.lineTo(e.x - s / 2, d), a.moveTo(e.x - s / 2, e.y);
      }
      ...
      if (a.closePath(), a.fill(), n.addLine) {
        ...
        if (a.beginPath(), a.setStrokeStyle(e.color), a.setLineWidth(n.width * t.pixelRatio), 1 === o.length) a.moveTo(o[0].x, o[0].y), a.arc(o[0].x, o[0].y, 1, 0, 2 * Math.PI);else {
          a.moveTo(o[0].x, o[0].y);
          let e = 0;

          if ("curve" === n.type) {
            for (let t, i = 0; i < o.length; i++) if (t = o[i], 0 == e && t.x > x && (a.moveTo(t.x, t.y), e = 1), 0 < i && t.x > x && t.x < c) {
              let e = createCurveControlPoints(o, i - 1);
              a.bezierCurveTo(e.ctrA.x, e.ctrA.y, e.ctrB.x, e.ctrB.y, t.x, t.y);
            }
          } else for (let t, i = 0; i < o.length; i++) t = o[i], 0 == e && t.x > x && (a.moveTo(t.x, t.y), e = 1), 0 < i && t.x > x && t.x < c && a.lineTo(t.x, t.y);
  ...
  return t._scrollDistance_ && 0 !== t._scrollDistance_ && !0 === t.enableScroll && (a.translate(t._scrollDistance_, 0), h = -t._scrollDistance_ - s + t.area[3], x = h + (t.xAxis.itemCount + 4) * s), e.forEach(function (e) {
    ...
    a.beginPath(), a.setStrokeStyle(e.color), a.setLineWidth(n.width), f.forEach(function (e) {
      if (1 === e.length) a.moveTo(e[0].x, e[0].y), a.arc(e[0].x, e[0].y, 1, 0, 2 * Math.PI);else {
        ...
        if ("curve" === n.type) {
          for (let o, n = 0; n < e.length; n++) if (o = e[n], 0 == i && o.x > h && (a.moveTo(o.x, o.y), i = 1), 0 < n && o.x > h && o.x < x) {
            var t = createCurveControlPoints(e, n - 1);
            a.bezierCurveTo(t.ctrA.x, t.ctrA.y, t.ctrB.x, t.ctrB.y, o.x, o.y);
          }
        } else for (let t, o = 0; o < e.length; o++) t = e[o], 0 == i && t.x > h && (a.moveTo(t.x, t.y), i = 1), 0 < o && t.x > h && t.x < x && a.lineTo(t.x, t.y);
...
function drawMixDataPoints(e, t, i, a) {
  ...
  if (t._scrollDistance_ && 0 !== t._scrollDistance_ && !0 === t.enableScroll && (a.translate(t._scrollDistance_, 0), c = Math.floor(-t._scrollDistance_ / r) - 2, p = c + t.xAxis.itemCount + 4, g = -t._scrollDistance_ - r + t.area[3], y = g + (t.xAxis.itemCount + 4) * r), e.forEach(function (e) {
    ...
    if (d.push(S), "column" == e.type) {
      ...
      for (let o, n = 0; n < S.length; n++) if (o = S[n], null !== o && n > c && n < p) {
        a.beginPath(), a.setStrokeStyle(o.color || e.color), a.setLineWidth(1), a.setFillStyle(o.color || e.color);
        var T = o.x - o.width / 2,
            A = t.height - o.y - t.area[2];
        a.moveTo(T, o.y), a.moveTo(T - 1, o.y), a.lineTo(T + o.width - 2, o.y), a.lineTo(T + o.width - 2, t.height - t.area[2]), a.lineTo(T, t.height - t.area[2]), a.lineTo(T, o.y), a.closePath(), a.stroke(), a.fill(), a.closePath(), a.fill();
      }
    ...
    if ("area" == e.type) {
      ...
      for (let n, l = 0; l < o.length; l++) {
        if (n = o[l], a.beginPath(), a.setStrokeStyle(e.color), a.setFillStyle(hexToRgb(e.color, .2)), a.setLineWidth(2 * t.pixelRatio), 1 < n.length) {
          var b = n[0];
          let t = n[n.length - 1];
          a.moveTo(b.x, b.y);
          let i = 0;
          ...
          if ("curve" === e.style) {
            for (let e, t = 0; t < n.length; t++) if (e = n[t], 0 == i && e.x > g && (a.moveTo(e.x, e.y), i = 1), 0 < t && e.x > g && e.x < y) {
              var P = createCurveControlPoints(n, t - 1);
              a.bezierCurveTo(P.ctrA.x, P.ctrA.y, P.ctrB.x, P.ctrB.y, e.x, e.y);
            }
          } else for (let e, t = 0; t < n.length; t++) e = n[t], 0 == i && e.x > g && (a.moveTo(e.x, e.y), i = 1), 0 < t && e.x > g && e.x < y && a.lineTo(e.x, e.y);

          a.lineTo(t.x, s), a.lineTo(b.x, s), a.lineTo(b.x, b.y);
        } else {
          let e = n[0];
          a.moveTo(e.x - r / 2, e.y), a.lineTo(e.x + r / 2, e.y), a.lineTo(e.x + r / 2, s), a.lineTo(e.x - r / 2, s), a.moveTo(e.x - r / 2, e.y);
        }
    ...
    if ("line" == e.type) {
      ...
      _.forEach(function (i) {
        ...
        if (a.beginPath(), a.setStrokeStyle(e.color), a.setLineWidth(2 * t.pixelRatio), 1 === i.length) a.moveTo(i[0].x, i[0].y), a.arc(i[0].x, i[0].y, 1, 0, 2 * Math.PI);else {
          ...
          if ("curve" == e.style) {
            for (let e, n = 0; n < i.length; n++) if (e = i[n], 0 == t && e.x > g && (a.moveTo(e.x, e.y), t = 1), 0 < n && e.x > g && e.x < y) {
              var o = createCurveControlPoints(i, n - 1);
              a.bezierCurveTo(o.ctrA.x, o.ctrA.y, o.ctrB.x, o.ctrB.y, e.x, e.y);
            }
          } else for (let e, o = 0; o < i.length; o++) e = i[o], 0 == t && e.x > g && (a.moveTo(e.x, e.y), t = 1), 0 < o && e.x > g && e.x < y && a.lineTo(e.x, e.y);
...
function drawYAxisGrid(e, t, a, i) {
  ...
  for (let o = 0; o < a.yAxisSplit + 1; o++) h.push(t.height - t.area[2] - n * o);

  i.save(), t._scrollDistance_ && 0 !== t._scrollDistance_ && i.translate(t._scrollDistance_, 0), "dash" == t.yAxis.gridType && i.setLineDash([t.yAxis.dashLength, t.yAxis.dashLength]), i.setStrokeStyle(t.yAxis.gridColor), i.setLineWidth(1 * t.pixelRatio), h.forEach(function (e) {
    i.beginPath(), i.moveTo(l, e), i.lineTo(l + d, e), i.stroke();
...
function drawYAxis(e, t, a, o) {
  ...
  for (let l = 0; l <= a.yAxisSplit; l++) h.push(t.area[0] + n * l);

  let x = t.area[3],
      c = t.width - t.area[1];

  for (let n, l = 0; l < t.yAxis.data.length; l++) if (n = t.yAxis.data[l], !0 !== n.disabled) {
    ...
    if (e.forEach(function (e, a) {
      var l = h[a] ? h[a] : s;
      o.beginPath(), o.setFontSize(i), o.setLineWidth(1 * t.pixelRatio), o.setStrokeStyle(n.axisLineColor || "#cccccc"), o.setFillStyle(n.fontColor || "#666666"), "left" == r.position ? (o.fillText(e + "", x - r.width, l + i / 2), !0 == n.calibration && (o.moveTo(x, l), o.lineTo(x - 3 * t.pixelRatio, l))) : (o.fillText(e + "", c + 4 * t.pixelRatio, l + i / 2), !0 == n.calibration && (o.moveTo(c, l), o.lineTo(c + 3 * t.pixelRatio, l))), o.closePath(), o.stroke();
    }), !1 !== n.axisLine && (o.beginPath(), o.setStrokeStyle(n.axisLineColor || "#cccccc"), o.setLineWidth(1 * t.pixelRatio), "left" == r.position ? (o.moveTo(x, t.height - t.area[2]), o.lineTo(x, t.area[0])) : (o.moveTo(c, t.height - t.area[2]), o.lineTo(c, t.area[0])), o.stroke()), t.yAxis.showTitle) {
      let e = n.titleFontSize || a.fontSize,
          i = n.title;
      o.beginPath(), o.setFontSize(e), o.setFillStyle(n.titleFontColor || "#666666"), "left" == r.position ? o.fillText(i, x - measureText(i, e) / 2, t.area[0] - 10 * t.pixelRatio) : o.fillText(i, c - measureText(i, e) / 2, t.area[0] - 10 * t.pixelRatio), o.closePath(), o.stroke();
    }
...
function drawLegend(e, t, i, a, o) {
  ...
  a.beginPath(), a.setLineWidth(t.legend.borderWidth), a.setStrokeStyle(t.legend.borderColor), a.setFillStyle(t.legend.backgroundColor), a.moveTo(r.start.x, r.start.y), a.rect(r.start.x, r.start.y, r.width, r.height), a.closePath(), a.fill(), a.stroke(), l.forEach(function (e, o) {
    ...
    for (let n, l = 0; l < e.length; l++) {
      switch (n = e[l], n.area = [0, 0, 0, 0], n.area[0] = y, n.area[1] = f, n.area[3] = f + p, a.beginPath(), a.setLineWidth(1 * t.pixelRatio), a.setStrokeStyle(n.show ? n.color : t.legend.hiddenColor), a.setFillStyle(n.show ? n.color : t.legend.hiddenColor), n.legendShape) {
        case "line":
      ...
      a.beginPath(), a.setFontSize(d), a.setFillStyle(n.show ? t.legend.fontColor : t.legend.hiddenColor), a.fillText(n.name, y, f + (.5 * p + .5 * d - 2)), a.closePath(), a.stroke(), "top" == t.legend.position || "bottom" == t.legend.position ? (y += measureText(n.name, d) + c, n.area[2] = y) : (n.area[2] = y + measureText(n.name, d) + c, y -= h + x, f += p);
    }
  });
}
...
function drawPieDataPoints(e, t, a, o) {
  ...
  if (!1 !== t.dataLabel && 1 === l) {
    for (var c = !1, p = 0, g = e.length; p < g; p++) if (0 < e[p].data) {
      c = !0;
      break;
    }
...
function drawArcbarDataPoints(e, t, i, a) {
  ...
  var r = {
    ...
    y: t.height / 2
      ...
      s = Math.min(r.x, r.y);
  s -= 5 * t.pixelRatio, s -= l.width / 2, a.setLineWidth(l.width), a.setStrokeStyle(l.backgroundColor || "#E9E9E9"), a.setLineCap("round"), a.beginPath(), "default" == l.type ? a.arc(r.x, r.y, s, l.startAngle * o, l.endAngle * o, !1) : a.arc(r.x, r.y, s, 0, 2 * o, !1), a.stroke();

  for (let n, d = 0; d < e.length; d++) n = e[d], a.setLineWidth(l.width), a.setStrokeStyle(n.color), a.setLineCap("round"), a.beginPath(), a.arc(r.x, r.y, s, l.startAngle * o, n._proportion_ * o, !1), a.stroke();
...
function drawGaugeDataPoints(e, t, a, i, o) {
  ...
  if ("progress" == r.type) {
    ...
    e.addColorStop("0", hexToRgb(t[0].color, .3)), e.addColorStop("1.0", hexToRgb("#FFFFFF", .1)), o.setFillStyle(e), o.arc(s.x, s.y, c, 0, 2 * n, !1), o.fill(), o.setLineWidth(r.width), o.setStrokeStyle(hexToRgb(t[0].color, .3)), o.setLineCap("round"), o.beginPath(), o.arc(s.x, s.y, h, r.startAngle * n, r.endAngle * n, !1), o.stroke(), x = r.startAngle - r.endAngle + 1;
    ...
    for (let e = 0; e < f; e++) o.beginPath(), u > e / f ? o.setStrokeStyle(hexToRgb(t[0].color, 1)) : o.setStrokeStyle(hexToRgb(t[0].color, .3)), o.setLineWidth(3 * a.pixelRatio), o.moveTo(g, 0), o.lineTo(y, 0), o.stroke(), o.rotate(p * n);
    ...
    for (let t, a = 0; a < e.length; a++) t = e[a], o.beginPath(), o.setStrokeStyle(t.color), o.arc(s.x, s.y, d, t._startAngle_ * n, t._endAngle_ * n, !1), o.stroke();
    ...
    for (let e = 0; e < r.splitLine.splitNumber + 1; e++) o.beginPath(), o.setStrokeStyle(r.splitLine.color), o.setLineWidth(2 * a.pixelRatio), o.moveTo(g, 0), o.lineTo(y, 0), o.stroke(), o.rotate(c * n);

    o.restore(), o.save(), o.translate(s.x, s.y), o.rotate((r.startAngle - 1) * n);

    for (let e = 0; e < r.splitLine.splitNumber * r.splitLine.childNumber + 1; e++) o.beginPath(), o.setStrokeStyle(r.splitLine.color), o.setLineWidth(1 * a.pixelRatio), o.moveTo(g, 0), o.lineTo(f, 0), o.stroke(), o.rotate(p * n);

    o.restore(), t = getGaugeDataPoints(t, e, r, l);

    for (let e, a = 0; a < t.length; a++) e = t[a], o.save(), o.translate(s.x, s.y), o.rotate((e._proportion_ - 1) * n), o.beginPath(), o.setFillStyle(e.color), o.moveTo(r.pointer.width, 0), o.lineTo(0, -r.pointer.width / 2), o.lineTo(-h, 0), o.lineTo(0, r.pointer.width / 2), o.lineTo(r.pointer.width, 0), o.closePath(), o.fill(), o.beginPath(), o.setFillStyle("#FFFFFF"), o.arc(0, 0, r.pointer.width / 6, 0, 2 * n, !1), o.fill(), o.restore();
...
function collisionNew(e, t, a, o) {
  ...
  for (let l = 0; l < t.length; l++) if (t[l].area) if (!(e[3] < t[l].area[1] || e[0] > t[l].area[2] || e[1] > t[l].area[3] || e[2] < t[l].area[0])) {
    n = !0;
    break;
...
function drawMapDataPoints(e, t, a, o) {
  ...
  for (var f = 0; f < d.length; f++) {
    ...
    for (var u = d[f].geometry.coordinates, m = 0; m < u.length; m++) {
      ...
      for (var S = 0; S < n.length; S++) l = coordinateToPoint(n[S][1], n[S][0], h, p, g, y), 0 == S ? (o.beginPath(), o.moveTo(l.x, l.y)) : o.lineTo(l.x, l.y);
    ...
    if (!0 == t.dataLabel) {
      ...
      if (T) {
        ...
        o.beginPath(), o.setFontSize(e), o.setFillStyle(d[f].textColor || "#666666"), o.fillText(t, l.x - measureText(t, e) / 2, l.y + e / 2), o.closePath(), o.stroke();
      }
    }
  }
  ...
  switch (t) {
    case "normal":
      for (let t = 0; t < a.length; t++) {
        ...
        for (;;) {
          d++, i = normalInt(-e.width / 2, e.width / 2, 5) - s / 2, o = normalInt(-e.height / 2, e.height / 2, 5) + r / 2, n = [i - 5 + e.width / 2, o - 5 - r + e.height / 2, i + s + 5 + e.width / 2, o + 5 + e.height / 2];
    ...
    case "vertical":
      ...
      for (let t = 0; t < a.length; t++) {
        ...
        for (;;) {
          c++;
          let t;
          if (x ? (i = normalInt(-e.width / 2, e.width / 2, 5) - h / 2, n = normalInt(-e.height / 2, e.height / 2, 5) + d / 2, l = [n - 5 - h + e.width / 2, -i - 5 + e.height / 2, n + 5 + e.width / 2, -i + d + 5 + e.height / 2], r = [e.width - (e.width / 2 - e.height / 2) - (-i + d + 5 + e.height / 2) - 5, e.height / 2 - e.width / 2 + (n - 5 - h + e.width / 2) - 5, e.width - (e.width / 2 - e.height / 2) - (-i + d + 5 + e.height / 2) + d, e.height / 2 - e.width / 2 + (n - 5 - h + e.width / 2) + h + 5], t = collisionNew(r, a, e.height, e.width)) : (i = normalInt(-e.width / 2, e.width / 2, 5) - h / 2, n = normalInt(-e.height / 2, e.height / 2, 5) + d / 2, l = [i - 5 + e.width / 2, n - 5 - d + e.height / 2, i + h + 5 + e.width / 2, n + 5 + e.height / 2], t = collisionNew(l, a, e.width, e.height)), !t) break;
...
function drawCharts(e, t, a, i) {
  ...
  if ("candle" == e) {
    ...
    e.show ? (s = calCandleMA(e.day, e.name, e.color, n[0].data), s = fillSeries(s, t, a), t.seriesMA = s) : t.seriesMA ? s = t.seriesMA = fillSeries(t.seriesMA, t, a) : s = n;
  } else s = n;
  ...
  if ("line" === t.type || "column" === t.type || "area" === t.type || "mix" === t.type || "candle" === t.type) {
    if (c = calYAxisData(n, t, a), p = c.yAxisWidth, t.yAxis.showTitle) {
      ...
      for (let o = 0; o < t.yAxis.data.length; o++) e = Math.max(e, t.yAxis.data[o].titleFontSize ? t.yAxis.data[o].titleFontSize : a.fontSize);
    ...
    for (let a = 0; a < p.length; a++) "left" == p[a].position ? (t.area[3] += 0 < o ? p[a].width + t.yAxis.padding : p[a].width, o += 1) : (t.area[1] += 0 < e ? p[a].width + t.yAxis.padding : p[a].width, e += 1);
  } else a.yAxisWidth = p;
  ...
  switch (("pie" === e || "ring" === e || "rose" === e) && (a._pieTextMaxLength_ = !1 === t.dataLabel ? 0 : getPieTextMaxLength(s)), e) {
    case "word":
      ...
      (!0 == t.updateData || null == t.updateData) && (t.chartData.wordCloudData = getWordCloudPoint(t, d.type)), this.animationInstance = new Animation({
        timing: "easeInOut",
        duration: r,
