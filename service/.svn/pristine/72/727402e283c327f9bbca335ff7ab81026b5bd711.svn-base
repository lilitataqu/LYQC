<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['running:userWeekStatistic:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['running:userWeekStatistic:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['running:userWeekStatistic:remove']"
        >删除</el-button>
      </el-col> -->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['running:userWeekStatistic:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="userWeekStatisticList" @selection-change="handleSelectionChange" @sort-change="sout" :default-sort = "{prop: 'distance', order: 'descending'}">
...
<script>
import { listUserWeekStatistic, userList } from "@/api/running/userWeekStatistic";

export default {
  name: "UserWeekStatistic",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUserWeekStatistic(this.queryParams).then(response => {
        this.userWeekStatisticList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加本周各学员运动量";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getUserWeekStatistic(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改本周各学员运动量";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateUserWeekStatistic(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addUserWeekStatistic(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除本周各学员运动量编号为"' + ids + '"的数据项？').then(function() {
        return delUserWeekStatistic(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('running/userWeekStatistic/export', {
        ...this.queryParams
      }, `userWeekStatistic_${new Date().getTime()}.xlsx`)
    },
    /** 排序监听事件 */
    sout(value){
      if(value.order == null){
